...
char* msg6 = "comhandProc dispatched";
int msgSize = 17;
int comSize = 25;
...
char* er6 = "comhandProc ran after it was terminated\n";
int erSize = 34;
int com_er_Size = 42;
...
void comhandProc()
{
  ...
  // repeat forever if termination fails
  sys_req(WRITE, DEFAULT_DEVICE, msg6, &comSize);
  comhand();
  // while(1){

  //   for(i=0; i<RC_6; i++){
      
  //     //calling comhand function to display command handler
      
  //     sys_req(IDLE, DEFAULT_DEVICE, NULL, NULL);
  //   }
  // }
  sys_req(EXIT, DEFAULT_DEVICE, NULL, NULL);
  sys_req(WRITE, DEFAULT_DEVICE, er6, &com_er_Size);
}